...
from thai_production_artifacts import build as verify_production
...
class BuildTimeShapingTests(unittest.TestCase):
 ...
 def test_signed_offsets_round_trip(self):
  ...
  self.assertEqual([(r["x_offset"]+r["contextual_shift_x"],r["y_offset"]+r["contextual_shift_y"]) for r in records],[(x,y) for _,x,y,_,_ in decoded])
 ...
 def test_all_glyph_ids_fit_allocated_font(self):
  indexes={int(e["compact_index"]) for e in self.mapping["glyphs"]};self.assertEqual(indexes,set(range(len(self.mapping["glyphs"]))))
 ...
 def test_contextual_clearance_selection_palette_and_assets(self):
  verify_production(draw_outputs=False)
 ...
 def test_compiled_rom_contains_positioned_stream(self):
  encoded,_,_=encode_run(PROOF_LINES[0],self.mapping);rom=ROOT/"pokeemerald.gba";self.assertTrue(rom.stat().st_mtime < MAP_PATH.stat().st_mtime or encoded+bytes([255]) in rom.read_bytes())
