# Thai build-time shaping report

## Status

Build-time shaping is implemented and the automated build result is **passed**.
...
|---|---:|---|
| 0 | 1 | `FC` extended-control introducer |
| 1 | 1 | `19` Thai positioned-glyph command |
...
Both C and assembly preprocessing rules in `Makefile` invoke the filter.
`--check` verifies the accepted proof trace, and a missing shaped glyph
...
`bitmap_x = round_half_away(HB_x_offset * 0.014 + bitmap_left)`

`bitmap_y = round_half_away(-HB_y_offset * 0.014 - bitmap_top)`

The renderer adds the encoded X offset to `cursorX` and the encoded Y
offset to `currentY + 12`, the common baseline. Advances use cumulative
...
`RenderThaiPositionedGlyph` in `src/text.c` validates the GBA glyph
...
- Final advance: 50px
- Bytes: `fc 19 16 00 00 f4 04 01 fc 19 0f 00 01 f5 09 01 fc 19 13 00 01 f8 04 01 fc 19 02 00 01 f8 08 01 fc 19 0a 00 01 f8 09 01 fc 19 0d 00 01 f8 08 01 fc 19 0c 00 01 f8 08 01`
...
- Final advance: 73px
- Bytes: `fc 19 01 00 01 f8 09 01 fc 19 1d 00 01 f8 06 01 fc 19 11 00 00 f8 06 01 fc 19 0a 00 01 f8 09 01 fc 19 19 00 01 f8 08 01 fc 19 11 00 00 f8 05 01 fc 19 0a 00 01 f8 09 01 fc 19 11 00 00 f8 06 01 fc 19 10 00 01 f8 07 01 fc 19 1c 00 01 f8 08 01`

## Tests and acceptance screen

`make test-thai-shaped-text` covers Unicode source readability, accepted
...
`data/text/birch_speech.inc` and contains all six strings. The New Game
label in `src/strings.c` also exercises the real C-source preprocessing
path.
